...
public final class CryptoUtilities {
    ...
    /**
     * ASK TA ABOUT THIS METHOD, I DO NOT THINKN IT MEETS THE CONTRACT'S
     * REQUIREMENTS
     */
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        NaturalNumber wSquared = new NaturalNumber2(w);
        powerMod(wSquared, two, n);
        if (wSquared.equals(one)) {
            isWitness = true;
        } else {
            NaturalNumber nMinusOne = new NaturalNumber2(n);
            nMinusOne.decrement();
            NaturalNumber wToNMinusOne = new NaturalNumber2(w);
            powerMod(wToNMinusOne, nMinusOne, n);
            if (!wToNMinusOne.equals(one)) {
                isWitness = true;
            }
        }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            ...
            NaturalNumber nDuplicate = new NaturalNumber2(n);
            nDuplicate.decrement();
            NaturalNumber one = new NaturalNumber2(1);
            for (int i = 0; i < fifty && isPrime; i++) {
                NaturalNumber random = new NaturalNumber2(
                        randomNumber(nDuplicate));
                while ((random.compareTo(one) <= 0)
                        || random.compareTo(nDuplicate) >= 0) {
                    random = randomNumber(nDuplicate);
                }
