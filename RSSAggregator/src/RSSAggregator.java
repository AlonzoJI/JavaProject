...
public final class RSSAggregator {
    ...
    public static void main(String[] args) {
        ...
        out.println(
                "Enter the name of the XML file containing the list of RSS feeds:");
        String xmlFileName = in.nextLine();
        out.println("Enter the name of the output HTML index file:");
        String indexFileName = in.nextLine();

        XMLTree xmlTree = new XMLTree1(xmlFileName);
        if (xmlTree.label().equals("feeds")) {
            SimpleWriter indexFileOut = new SimpleWriter1L(indexFileName);

            String indexTitle = xmlTree.hasAttribute("title")
                    ? xmlTree.attributeValue("title")
                    : "RSS Feed Index";
            indexFileOut.println("<html><head><title>" + indexTitle
                    + "</title></head><body>");
            indexFileOut.println("<h1>" + indexTitle + "</h1>");
            ...
            for (int i = 0; i < xmlTree.numberOfChildren(); i++) {
                ...
                if (feed.isTag() && feed.label().equals("feed")) {
                    ...
                    indexFileOut.println("<li><a href=\"" + feedFile + "\">"
                            + feedName + "</a></li>");
                }
            }

            indexFileOut.println("</ul></body></html>");
            indexFileOut.close();
        } else {
            out.println(
                    "The provided file does not have the correct structure.");
        }
