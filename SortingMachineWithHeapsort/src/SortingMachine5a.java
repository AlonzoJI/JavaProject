...
public class SortingMachine5a<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(T[] array, int top, int last,
        ...
        /*
         * Calculate indices for the left and right children of the current root
         */
        int left = 2 * top + 1;
        int right = left + 1;

        /*
         * If the left child is the last element in the heap, compare it with
         * the root. If the root is greater, swap them to maintain the heap
         * property.
         */
        if (left == last) {
         ...
         * smaller one and swap it with the root if necessary. Then recursively
         * sift down the child that was swapped with the root.
         */
        else if (right <= last) {
    ...
    private static <T> void heapify(T[] array, int top, Comparator<T> order) {
        ...
        // Calculate indices for the left and right children of the current node
        int left = 2 * top + 1;
        int right = left + 1;

        // Recursively heapify the left and right subtrees if they exist
    ...
    private static <T> T[] buildHeap(Queue<T> q, Comparator<T> order) {
        ...
        // Dequeue all elements from the queue and place them in the heap array
        for (int i = 0; i < length; i++) {
            heap[i] = q.dequeue();
        }

        // The array is not yet a valid heap, so we need to heapify it
    ...
    private void createNewRep(Comparator<T> order) {
        ...
        // Set the machineOrder to the provided comparator
        this.machineOrder = order;

        // Enable insertion mode
        this.insertionMode = true;

        // Initialize an empty queue for entries
    ...
    public final T removeFirst() {
        ...
        T first = this.heap[0];
        /*
         * now since from the client view first is no more in the heap we can
         * move the element to the end of the array and store it in the junk
         * part
         *
         * by this we can exchange the last entry with the root and sift it down
         */
