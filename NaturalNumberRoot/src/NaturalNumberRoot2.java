...
public final class NaturalNumberRoot2 {
    ...
    public static void root(NaturalNumber n, int r) {
        ...
        int log10n = n.toString().length();
        int k = (log10n - 1) / r;

        NaturalNumber lowEnough = new NaturalNumber2(10);
        lowEnough.power(k);
        NaturalNumber tooHigh = new NaturalNumber2(lowEnough);
        tooHigh.multiply(new NaturalNumber2(10));
        ...
        boolean isZeroOrOne = n.isZero() || n.compareTo(one) == 0;

        if (!isZeroOrOne) {
            while (difference.compareTo(one) > 0) {
                guessRoot.copyFrom(lowEnough);
                guessRoot.add(tooHigh);
                guessRoot.divide(two);

                NaturalNumber guessValue = new NaturalNumber2(guessRoot);
                guessValue.power(r);

                if (guessValue.compareTo(n) <= 0) {
