...
public final class StringReassembly {
    ...
    public static int overlap(String str1, String str2) {
         ...
         */
        int maxPossibleOverlap;
        if (str1.length() < str2.length()) {
            maxPossibleOverlap = str1.length();
        } else {
            maxPossibleOverlap = str2.length();
        }

        int maxOverlap = 0;
        boolean found = false;
        for (int possibleOverlap = maxPossibleOverlap; possibleOverlap > 0
                && !found; possibleOverlap--) {
            if (str1.regionMatches(str1.length() - possibleOverlap, str2, 0,
                    possibleOverlap)) {
                maxOverlap = possibleOverlap;
                found = true;
            }
        }
        return maxOverlap;
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        String combination = str1.substring(0, str1.length() - overlap) + str2;
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        boolean shouldAddStr = true;

        for (String stringInSet : strSet) {
            if (stringInSet.contains(str)) {
                shouldAddStr = false;
            }
        }

        if (shouldAddStr) {
            Set<String> substringsToRemove = strSet.newInstance();
            for (String stringInSet : strSet) {
                if (str.contains(stringInSet)) {
                    substringsToRemove.add(stringInSet);
                }
            }
            strSet.remove(substringsToRemove);
            strSet.add(str);
        }
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        Set<String> linesWithoutSubstrings = new Set1L<>();
        while (!input.atEOS()) {
            String currentLine = input.nextLine();
            addToSetAvoidingSubstrings(linesWithoutSubstrings, currentLine);
        }

        return linesWithoutSubstrings;
    }
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        int index = 0;
        int tildeIndex = text.indexOf('~', index);

        while (tildeIndex != -1) {
            out.println(text.substring(index, tildeIndex));
            index = tildeIndex + 1;

            tildeIndex = text.indexOf('~', index);
        }

        out.print(text.substring(index));
